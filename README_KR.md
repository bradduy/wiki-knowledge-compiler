...
### v0.4.0 — 보존 감쇠, 결정화, 품질 점수
- 📉 **보존 감쇠** — 페이지에 접근하거나 강화하지 않으면 신뢰도가 시간이 지남에 따라 감소합니다. 각 페이지 유형마다 고유한 반감기가 있습니다 (개념: 180일, 인사이트: 60일, 초안: 30일). `/wiki`에서 페이지를 읽으면 타이머가 초기화됩니다.
- 💎 **결정화** — `/wiki`가 3개 이상의 출처를 종합하면, 질문, 답변, 출처, 주요 연결, 교훈을 담은 **다이제스트 페이지**를 자동 생성합니다 — 연구를 영구적인 위키 페이지로 전환합니다.
- 📊 **품질 점수** — 모든 페이지가 5가지 차원(프론트매터, 인용, 최신성, 관계, 콘텐츠)에서 0–100으로 평가됩니다. 등급 A–F. 60점 미만 페이지는 `/wiki-update`와 `/wiki-audit`에서 표시됩니다.

### v0.3.0 — 위키 감사, 프라이버시 필터, 모순 해결
- 🔍 **`/wiki-audit`** — 위키 건강 상태를 점검하는 새 명령어: 깨진 링크, 고아 페이지, 오래된 콘텐츠, 누락된 프론트매터, 인용 불일치, 모순, 엔티티 그래프 문제. 가능한 것은 자동 수정하고, 나머지는 품질 점수 요약과 함께 보고합니다.
- 🔒 **프라이버시 필터** — 인제스트 중 파생 콘텐츠에서 API 키, 토큰, 비밀번호, 개인정보를 자동 제거합니다. 원본 소스는 절대 수정되지 않습니다. 페이지에 `visibility: public/private/internal` 태그가 지정됩니다.
- ⚖️ **모순 해결** — 출처가 불일치할 때 양쪽 모두 최신성, 권위성, 증거 수로 점수가 매겨집니다. 더 강한 주장이 약한 주장을 대체하고, 비슷한 경우 사람의 검토를 위해 표시됩니다.
...
| **보존 감쇠** | 강화되지 않으면 신뢰도가 시간이 지남에 따라 감소 — 페이지 유형별 고유 반감기 |
| **품질 점수** | 모든 페이지가 5가지 차원에서 0–100으로 평가, 등급 A–F |
| **결정화** | 다중 출처 답변이 재사용 가능한 다이제스트 페이지로 자동 증류 |
| **위키 감사** | `/wiki-audit`가 깨진 링크, 오래된 콘텐츠, 모순을 찾아 자동 수정 |
| **프라이버시 필터** | API 키, 토큰, 비밀번호, 개인정보를 파생 콘텐츠에서 자동 제거 |
| **모순 해결** | 충돌하는 주장이 최신성/권위성/증거로 점수화, 해결 또는 표시 |
...
| `/wiki-audit` | 🩺 위키 건강 점검 — 깨진 링크 수정, 모순 감지, 품질 점수 평가 |

**권장 순서:** `/wiki-setup` → `/wiki-ingest` → `/wiki` → `/wiki-insights` → `/wiki-update` → `/wiki-audit` → `/wiki-schedule`
