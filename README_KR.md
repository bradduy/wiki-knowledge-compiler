...
플러그인은 키워드 검색만 하지 않습니다. **지식 그래프**를 탐색합니다 — "탄소 예산" 개념에서 시작하여 관련 엔티티와 주제로의 관계를 따라가며, 키워드 검색이 놓치는 연결을 찾습니다.
...
"Redis 업그레이드의 영향은?"이라고 물으면, Redis 노드에서 `uses`, `depends-on`, `maintained-by` 엣지를 따라 나가며 관련된 모든 서비스, 사람, 결정을 찾습니다.
