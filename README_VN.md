...
> **Nâng cấp từ v0.0.x?** Chạy `/wiki-setup` lại để khởi tạo thư mục `.data/entities/` mới. Các trang wiki hiện tại vẫn hoạt động bình thường — các trường mới (`confidence`, `verified`, `entities`, `related` phân loại) được thêm dần khi bạn nhập tài liệu mới hoặc chạy `/wiki-update`. Không cần di chuyển dữ liệu.
...
| Loại thực thể | Ví dụ |
|-------------|---------|
| Người | "Brad Duy", "Andrej Karpathy" |
