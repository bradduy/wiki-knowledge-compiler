...
### v0.4.0 — 留存衰减、结晶化和质量评分
- 📉 **留存衰减** — 可信度随时间衰减，除非页面被访问或强化。每种页面类型有自己的半衰期（概念：180天，洞察：60天，草稿：30天）。在 `/wiki` 中阅读页面会重置计时。
- 💎 **结晶化** — 当 `/wiki` 综合 3 个以上来源时，自动生成**摘要页面**，包含问题、答案、来源、关键联系和经验教训——将你的研究转化为永久的 wiki 页面。
- 📊 **质量评分** — 每个页面在 5 个维度（前置元数据、引用、新鲜度、关系、内容）上评分 0–100。等级 A–F。低于 60 分的页面在 `/wiki-update` 和 `/wiki-audit` 中被标记。

### v0.3.0 — Wiki 审计、隐私过滤和矛盾解决
- 🔍 **`/wiki-audit`** — 新命令，检查 wiki 健康状况：断链、孤立页面、过期内容、缺失前置元数据、引用不匹配、矛盾、实体图谱问题。自动修复可修复的部分，其余以质量评分摘要形式报告。
- 🔒 **隐私过滤** — 在导入过程中自动从衍生内容中剥离 API 密钥、令牌、密码和个人身份信息。原始来源永不修改。页面标记为 `visibility: public/private/internal`。
- ⚖️ **矛盾解决** — 当来源不一致时，双方按新近度、权威性和证据数量评分。较强的主张取代较弱的；势均力敌的情况标记供人工审查。
...
| **留存衰减** | 可信度随时间衰减，除非被强化——每种页面类型有自己的半衰期 |
| **质量评分** | 每个页面在 5 个维度上评分 0–100，等级 A–F |
| **结晶化** | 多来源回答自动提炼为可复用的摘要页面 |
| **Wiki 审计** | `/wiki-audit` 查找断链、过期内容、矛盾并自动修复 |
| **隐私过滤** | 自动从衍生内容中剥离 API 密钥、令牌、密码和个人身份信息 |
| **矛盾解决** | 冲突的声明按新近度/权威性/证据评分，解决或标记 |
...
| `/wiki-audit` | 🩺 检查 wiki 健康状况 — 修复断链、检测矛盾、评估质量 |

**推荐顺序：** `/wiki-setup` → `/wiki-ingest` → `/wiki` → `/wiki-insights` → `/wiki-update` → `/wiki-audit` → `/wiki-schedule`
